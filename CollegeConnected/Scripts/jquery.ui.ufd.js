    ...
    $.widget(widgetName,
    {
        ...
        _create: function() { //1.8 init
            ...
            this.wrapper = $([
                ...
                '<button type="button" tabindex="-1" class="', css.button, '"><div class="', css.buttonIcon,
                '"/></button>',
                //   <select .../> goes here
                "</span>"
            ].join(""));
        ...
        _initEvents: function() { //initialize all event listeners
            ...
            this.input.bind("keydown keypress keyup",
                function(event) {
            ...
            */
            this.listScroll.bind("DOMMouseScroll mousewheel",
                function(e) {
                    self.stopEvent(e);
                    e = e ? e : window.event;
                    // Fix from https://code.google.com/p/ufd/issues/detail?id=60 for mouse only scrolling one direction.
            ...
            this.listScroll.bind("mouseover mouseout click",
                function(e) {
            ...
            this.selectbox.bind("change." + widgetName,
                function(e) {
        ...
        */
        ...
        _populateFromMaster: function() {
            ...
            while (loopCountdown--) {
                thisOpt = options[index];
                self.trie.add($.trim(thisOpt
                        .text),
                    theLiSet[index++]); //option.text not innerHTML for trie as we dont want escaping
            }
...
/* end InfixTrie */
    ...
    $.extend($.ui.ufd,
    {
        ...
        defaults: { // 1.7 default options location, see below
            ...
            dropDownID: "ufd-container",
// ID for a root-child node for storing dropdown lists. avoids ie6 zindex issues by being at top of tree.
...
//copy these properties to widget. Width auto-copied unless min/manual.
            ...
            submitFreeText: false,
// re[name] original select, give text input the selects' original [name], and allow unmatched entries  
            ...
            unwrapForCSS: false,
// unwrap select on reload to get % right on units etc. unwrap causes flicker on reload in iE6
            listWidthFixed: true,
// List width matches widget? If false, list can be wider to fit item width, but uses min-width so no iE6 support.  

            polling: 250,
// poll msec to test disabled, dimensioned state of master. 0 to disable polling, but needed for (initially) hidden fields. 
            ...
            manualWidth: null,
//override selectbox width; set explicit width - stops flicker on reload in iE6 (unless unwrapForCSS) as no unwrap needed
            viewAhead: 1, // items ahead to keep in view when cursor scrolling
            pageLength: 10, // number of visible items jumped on pgup/pgdown.
            delayFilter: ($.support
                    .style)
                ? 1
                : 150, // msec to wait before starting filter (or get cancelled); long for IE 
